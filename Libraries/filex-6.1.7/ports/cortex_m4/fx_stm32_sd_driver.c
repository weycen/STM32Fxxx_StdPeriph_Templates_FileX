...
UINT SD_ReadBlocks(uint8_t* pBuffer, uint64_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks) {
  if(NumberOfBlocks == 1) {
    return (UINT)SD_ReadBlock(pBuffer, ReadAddr, BlockSize);
  } else {
    return (UINT)SD_ReadMultiBlocks(pBuffer, ReadAddr, BlockSize, NumberOfBlocks);
  }
}

UINT SD_WriteBlocks(uint8_t* pBuffer, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks) {
  if(NumberOfBlocks == 1) {
    return (UINT)SD_WriteBlock(pBuffer, WriteAddr, BlockSize);
  } else {
    return (UINT)SD_WriteMultiBlocks(pBuffer, WriteAddr, BlockSize, NumberOfBlocks);
  }
}
